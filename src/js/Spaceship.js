 class Spaceship extends ResizableObject {
    constructor() {
        ...
        this.velocity = {
            distanceToAddX: 0, distanceToAddY: 0,
        ...
        this.bonusTimer = new THREE.Mesh(new THREE.BoxGeometry(1, 1, 1), new THREE.MeshBasicMaterial({color: 0xffffff}));
        this.bonusTimer.position.set(this.position.x, this.position.y - this.size.y, 0);

        //this.displayBonusTimer();
    }
    ...
    update() {
        ...
        if(eventHandler.keys[37]) this.velocity.arl = gameParameters.spaceship.rotationSpeed; else this.velocity.arl = 0;
        if(eventHandler.keys[39]) this.velocity.arr = -gameParameters.spaceship.rotationSpeed; else this.velocity.arr = 0;
